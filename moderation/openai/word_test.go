...
func TestOpenAIClientAgainstWords(t *testing.T) {
	...
	testWordFlagging(t, client)
}

func testWordFlagging(t *testing.T, client moderation.ModerationClient) {
	safeCases := createTestCases(safeWordListEn, false)
	unsafeCases := createTestCases(unsafeWordListEn, true)
	testCases := append(safeCases, unsafeCases...)

	for _, tc := range testCases {
		tc := tc
		t.Run(tc.phrase, func(t *testing.T) {
			// Classify the text using the moderation client
			result, err := client.ClassifyText(tc.phrase)
			if err != nil {
				t.Fatalf("unexpected error for word '%s': %v", tc.phrase, err)
			}
